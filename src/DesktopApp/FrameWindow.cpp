...
IMPLEMENT_DYNAMIC(FrameWindow, CFrameWnd)
...
FrameWindow::FrameWindow():b_maximized(FALSE)
{
	//Set the windows frame parameters
	winStats.bdr_wdth =0;
	winStats.frm_clp_wdth = 0;
	winStats.titlb_sz = 30;
...
#ifdef _DEBUG
void FrameWindow::AssertValid() const
{
	CFrameWnd::AssertValid();
}

void FrameWindow::Dump(CDumpContext& dc) const
{
	CFrameWnd::Dump(dc);
}
...
BOOL FrameWindow::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	CRect rect;
	GetClientRect(rect);
	...
	m_CustomRgn.CreateRoundRectRgn(rect.left + winStats.frm_clp_wdth, 
		...
		, rect.right - winStats.frm_clp_wdth, rect.bottom - winStats.frm_clp_wdth, 0,0);
    VERIFY(SetWindowRgn(m_CustomRgn , TRUE ));
	st_winNm.Create("Custom Window", WS_CHILD | WS_VISIBLE, 
...
BOOL FrameWindow::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle , CWnd* pParentWnd , CCreateContext* pContext)
{
...
void FrameWindow::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void FrameWindow::OnMouseMove(UINT nFlags, CPoint point)
{
...
void FrameWindow::OnSize(UINT nType, int cx, int cy)
{
	...
	m_CustomRgn.CreateRoundRectRgn(rect.left + winStats.frm_clp_wdth, 
		...
		rect.bottom - winStats.frm_clp_wdth, 0, 0);
...
int FrameWindow::ProcessResize(CPoint point)
{
	int Val = -1;
...
BOOL FrameWindow::OnEraseBkgnd(CDC* pDC)
{
...
void FrameWindow::restore(void)
{
	MoveWindow(m_PrevRect);
	b_maximized = FALSE;
}

void FrameWindow::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (r_windRects.r_Move.PtInRect(point)){
		if (!b_maximized)
			maximize();
...
void FrameWindow::close_window(void)
{
	DestroyWindow();
}

void FrameWindow::SetWindowRects(void)
{
...
bool FrameWindow::HandleCutomeFrame(CPoint point)
{
	CRect rect, rRect;
	GetClientRect(rect);
...
void FrameWindow::HandleCustomeFrameCursor(CPoint point)
{
...
void FrameWindow::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
...
HBRUSH FrameWindow::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
